...
from programming.generators.models_fastchat import fschat_instruct_conv, fschat_instruct_prompt
import dataclasses
...
class FastChatModel(ModelBase):
    def __init__(self, name):
        if 'llama3' in name:
            self.conv = get_conv_template('llama-3')
        else:
            raise NotImplementedError
        super().__init__(name)
    ...
    def generate_chat(self, messages: List[Message], max_tokens: int = 1024, temperature: float = 0.2, num_comps: int = 1) -> Union[List[str], str]: # todo
        # def fschat_instruct_conv(conv, model, temperature, max_new_tokens, n, stop) -> list:
        raise NotImplementedError

    def generate(self, prompt: str, max_tokens: int = 1024, stop_strs: Optional[List[str]] = None, temperature: float = 0.0, num_comps=1) -> Union[List[str], str]:
        stop_set = set()
        stop_set.add(self.conv.stop_str)
        if stop_strs is not None:
            for stop_str in stop_strs:
                stop_set.add(stop_str)
        responses = fschat_instruct_prompt(prompt, self.name, temperature, max_tokens, 1, stop=list(stop_set), stop_ids=self.conv.stop_token_ids)
