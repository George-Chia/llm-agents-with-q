    ...
    for idx, item in enumerate(dataset):
        ...
        for cur_iter in range(max_iters):
            ...
            for _ in range(n):
                ...
                conditional_context = ''
                if use_condition and len(prev_solutions) > 0:
                    conditional_context += '\n\nBelow are the potential implementations you might generate along with their corresponding unit test results: \n\n'
                    conditional_context += "\n\n".join(
                            [
                                f"[previous impl {i + 1}]:\n{impl}\n[unit test results from previous impl {i + 1}]:\n{feedback}\n"
                                for i, (impl, feedback) in
                                enumerate(zip(prev_solutions, pred_feedback_internal))]
                        )
                    conditional_context += 'Please summarize insights from the potential implementations and unit test feedback, and generate a new implementation with as much distinctiveness as possible.\n'
                while new_solution is None:
                    new_solution = gen.func_impl(
                        ...
                        conditional_text = conditional_context
                    )
                ...
                for child in node.children:
                    ...
                    prev_solutions.append(child.solution)
                    pred_feedback_internal.append(feedback_internal)
