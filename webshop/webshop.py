...
class WebShopTask(Task):
    ...
    # @staticmethod
    # def fastchat_eto_prompt_wrap(x: str, y: str = '', reflection_mapping_list=[]):
    #     cur_task=x
    #     observation, fschat_messages = prompt_with_icl(instruction, raw_icl, cur_task, 1)

    @staticmethod
    def react_prompt_wrap(x: str, y: str = ''):
        # ETO和这个环境里的observation格式不一样，ICL没法直接迁移过来用；
        # 答：已修改好，采用fastchat的对话格式（见上一个函数）
        raise NotImplementedError 
        question = x
        input = x + y
        trajectories = ""
        return prompt_react.format(input=input)
