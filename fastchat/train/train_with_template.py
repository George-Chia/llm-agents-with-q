...
class ModelArguments:
    ...
    conv_template: Optional[str] = field(default=None)
...
def apply_prompt_template(sources, template_id, systems=None, conv_template=None):
...
def get_prompt_separator(conv):
    # print("conv.sep_style: ", conv.sep_style)
...
def preprocess(
    sources, tokenizer: transformers.PreTrainedTokenizer, template_id, conv_template, **kwargs
) -> Dict:
    ...
    if len(sources) <= 1000:
        conversations, conv = apply_prompt_template(sources, template_id, systems, conv_template)
        ...
        with Pool() as p:
            conversations, conv = p.apply_async(
                apply_prompt_template, (sources, template_id, systems, conv_template)
            ).get()
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        self, raw_data, tokenizer: transformers.PreTrainedTokenizer, template_id, conv_template
    ):
        ...
        data_dict = preprocess(sources, tokenizer, template_id, conv_template, systems=systems)
...
def make_supervised_data_module(
    ...
    train_dataset = dataset_cls(
        train_raw_data, tokenizer=tokenizer, template_id=template_id,conv_template=conv_template
    )
    eval_dataset = dataset_cls(
        eval_raw_data, tokenizer=tokenizer, template_id=template_id,conv_template=conv_template
    )
...
def train():
    ...
    conv_template = model_args.conv_template
