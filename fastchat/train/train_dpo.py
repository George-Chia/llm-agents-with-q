...
def mask_labels(conversation, target, tokenizer, conv):
    # aa = tokenizer(conversation, return_tensors="pt", max_length=tokenizer.model_max_length, truncation=True)['input_ids'][0]
    ...
    if conv.sep2==conv.sep:
        raise NotImplementedError
        turns = conversation.split(conv.sep2+'\n'+conv.roles[0])
        assert len(conversation.split(conv.sep2+'\n'+conv.roles[0])) == len(conversation.split(conv.sep2))//2
    else:  
        turns = conversation.split(conv.sep2)
    cur_len = 1
    ...
    for i, turn in enumerate(turns):
        ...
        # if conv.name == 'phi3': #  do not know why, but add this is ok
        #     instruction_len += 1
        #     if i != 0:
        #         cur_len += 1
        ...
        if conv.sep2 == '</s>':
            ...
            cur_len += turn_len + 3
        elif conv.sep2 == '<|end|> <|end_of_turn|>':
            cur_len += turn_len + 15
        else:
            raise NotImplementedError
...
def train():
    ...
    train_dataset = dataset["train"].map(preprocess, cache_file_name=None)
