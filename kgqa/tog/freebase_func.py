...
def half_stop(question, cluster_chain_of_entities, depth, args, retrivainfo):
    print("No new knowledge added during search depth %d, stop searching." % depth)
    answer = generate_answer(question, cluster_chain_of_entities, args, retrivainfo)
    save_2_jsonl(question, answer, cluster_chain_of_entities, retrivainfo, file_name=args.dataset)


def generate_answer(question, cluster_chain_of_entities, args, retrivainfo):
...
def process_search(question, cluster_chain_of_entities, obs, args):
    chain_prompt = '\n'.join([', '.join([str(x) for x in chain]) for sublist in cluster_chain_of_entities for chain in sublist])
    prompt = prompt_search + question
    prompt += "\nKnowledge Triplets: " + chain_prompt + '\nretrivainfo: ' + obs + 'keywords: '
    query = run_llm(prompt, args.temperature_reasoning, args.max_length, args.opeani_api_keys, args.LLM_type)
